...
export function TeacherLessonPublisher({ draftId }: { draftId: string }) {
  ...
  const [audience, setAudience] = useState<DailyLessonAudience>('public')
  ...
  return <section className="teacher-publish-panel">
    <div className="teacher-publish-title"><CalendarClock/><div><p className="eyebrow">REVIEW → SCHEDULE → PUBLISH</p><h3>Julkaise vasta tarkistuksen jälkeen</h3></div></div>
    <div className="teacher-publish-fields">
      <label>Taito<select value={skill} onChange={event => setSkill(event.target.value as DailyLessonSkill)}>{['mixed','speaking','listening','reading','writing','understanding','vocabulary','yki-test'].map(item=><option key={item} value={item}>{item}</option>)}</select></label>
