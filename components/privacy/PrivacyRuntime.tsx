...
export default function PrivacyRuntime() {
  ...
  return (
    <>
      {process.env.NODE_ENV === 'production' && consent === 'all' ? <Analytics /> : null}
      {consent === null ? (
        <div className="cookie-banner" role="dialog" aria-label="Evästeasetukset" aria-live="polite">
          <div>
            <strong>Yksityisyys ensin.</strong>
            <p>
              OpiOpe käyttää välttämättömiä tallennuksia palvelun toimintaan. Analytiikka käynnistyy vain, jos hyväksyt sen.
              <a href="/cookies"> Lue evästeistä</a>.
            </p>
          </div>
          <div className="cookie-actions">
            <button className="outline-button" onClick={() => save('essential')}>Vain välttämättömät</button>
            <button className="primary-button" onClick={() => save('all')}>Hyväksy analytiikka</button>
          </div>
        </div>
      ) : null}
    </>
