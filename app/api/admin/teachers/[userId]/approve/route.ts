...
export async function POST(request: Request, context: { params: Promise<{ userId: string }> }) {
  ...
  const { data, error } = await access.admin.rpc('approve_teacher_application', {
    p_user_id: userId,
    p_admin_user_id: access.userId,
  })

  if (error) return NextResponse.json({ error: 'Opettajaoikeuden aktivointi epäonnistui.' }, { status: 500 })
